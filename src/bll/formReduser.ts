...
const slice = createSlice({
  ...
  reducers: {
    addPlantAC(state, action: PayloadAction<{ cards: CardsType }>) {
      const redPlant = action.payload.cards.red.filter((card) => card.isDone).map((c) => c.title)
      ...
      const greenPlant = action.payload.cards.green
        .filter((card) => card.isDone)
        .map((c) => c.title)
      const bluePlant = action.payload.cards.blue.filter((card) => card.isDone).map((c) => c.title)
